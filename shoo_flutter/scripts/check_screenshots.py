...
for s in r.json().get('data',[]):
    ...
    sc=requests.get(f'https://api.appstoreconnect.apple.com/v1/appScreenshotSets/{s["id"]}/appScreenshots',headers=H)
    print(f'{dt}:')
    for ss in sc.json().get('data',[]):
        fn=ss['attributes']['fileName']
        state=ss['attributes'].get('state','?')
        print(f'  {fn}: {state}')
