...
for loc in locs:
    ...
    for sset in r.json().get('data', []):
        ...
        for ss in r.json().get('data', []):
            ...
            if state != 'COMPLETE':
                bad.append(f"{locale}/{dt}/{ss['attributes']['fileName']}: {state}")
    ...
    for pset in r.json().get('data', []):
        ...
        for pv in r.json().get('data', []):
            state = pv['attributes'].get('assetDeliveryState', {}).get('state', '?')
            if state != 'COMPLETE':
                bad.append(f"{locale}/video/{pt}/{pv['attributes']['fileName']}: {state}")
