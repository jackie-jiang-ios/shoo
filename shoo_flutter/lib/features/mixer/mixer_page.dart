...
class _MixerPageState extends ConsumerState<MixerPage> {
  ...
  void _addSound(RecommendedSound sound, String animalName) {
    if (_mixItems.any((item) => item.sound.name == sound.name)) return;
    setState(() {
  ...
  Widget _buildEmptyState(ThemeData theme, S s) {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.merge_type, size: 80, color: AppColors.textHint),
      const SizedBox(height: 16),
      Text(s.addSound, style: theme.textTheme.titleMedium?.copyWith(color: AppColors.textSecondary)),
  ...
  Widget _buildMixContent(ThemeData theme, S s) {
    return Column(children: [
      Container(
        ...
        decoration: const BoxDecoration(gradient: LinearGradient(colors: [AppColors.primaryDark, AppColors.primary]), borderRadius: BorderRadius.all(Radius.circular(20))),
        child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(_mixItems.map((i) => s.isZh ? i.sound.name : i.sound.nameEn).join(' + '),
      ...
      Expanded(child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final soundName = s.isZh ? item.sound.name : item.sound.nameEn;
          return Card(
            margin: const EdgeInsets.symmetric(vertical: 4),
            child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
              Row(children: [
                Expanded(child: Text(soundName, style: theme.textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.w600))),
                Text(item.animalName, style: theme.textTheme.bodySmall?.copyWith(color: AppColors.textSecondary)),
    ...
    return SafeArea(child: Padding(padding: const EdgeInsets.all(16),
      child: FilledButton.icon(
        ...
        style: FilledButton.styleFrom(
          backgroundColor: _isAnyPlaying ? AppColors.danger : AppColors.primary,
            ...
            Expanded(child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ExpansionTile(
                  ...
                  subtitle: Text(s.isZh ? animal.description : animal.descriptionEn, style: const TextStyle(fontSize: 12)),
                  children: animal.sounds.map((sound) {
                    final isAdded = _mixItems.any((i) => i.sound.name == sound.name);
